...
function skinsFlags()
    ...
    for start = 1, tablelength(db), 1 do
        if db[start].type == "skin" then
            skinsResult[numero] = db[start]
            skinsChoice[numero] = db[start].name
            numero = numero + 1
        end
...
function itensFlags()
    ...
    numero = 1
    for start = 1, tablelength(db), 1 do
        if db[start].type == "iten" then
            itensResult[numero] = db[start]
            itensChoice[numero] = db[start].name
            numero = numero + 1
        end
        if db[start].type == "sair" then
            itensChoice[numero] = db[start].name
...
function decoFlags()
    ...
    numero = 1
    for start = 1, tablelength(db), 1 do
        if db[start].type == "decor" then
            decoResult[numero] = db[start]
            decoChoice[numero] = db[start].name
            numero = numero + 1
        end
        if db[start].type == "sair" then
            decoChoice[numero] = db[start].name
...
function fotoFlags()
    ...
    numero = 1
    for start = 1, tablelength(db), 1 do
        if db[start].type == "foto" then
            fotoResult[numero] = db[start]
            fotoChoice[numero] = db[start].name
            numero = numero + 1
        end
        if db[start].type == "sair" then
            fotoChoice[numero] = db[start].name
...
-- decoracaoResult[numeror] = decoResult[numeror].name
-- end

-- itensResult = {}

-- for numeror=1, tablelength(itensResult), 1 do
-- itensResult[numeror] = itensResult[numeror].name
-- end
-- end
...
function toolsGame()
    ...
    if toolsGameM == 4 then
        armazemUp()
    end
    if toolsGameM == 05 then
        mainMenu()
    end
end
...
function skinsMenu()
    local MenuSkins = gg.choice(skinsChoice, nil, "ϟSelecione uma skin!ϟ")
    if MenuSkins == nil then
    else
        if MenuSkins == tablelength(skinsChoice) then
            reflashFlags(SearchTag)
        else
            valorFlag(
                skinsResult[MenuSkins].value1,
                skinsResult[MenuSkins].value2,
                skinsResult[MenuSkins].value3,
                skinsResult[MenuSkins].value4,
                skinsResult[MenuSkins].value5,
                skinsResult[MenuSkins].value6,
                skinsResult[MenuSkins].value7,
                skinsResult[MenuSkins].flagsOn
            )
            reflashFlags(SearchTag)
        end
    end
...
function decoracaoMenu()
    local Menudecoracao = gg.choice(decoChoice, nil, "ϟSelecione uma decoração!ϟ")
    if Menudecoracao == nil then
    else
        if Menudecoracao == tablelength(decoChoice) then
            reflashFlags(SearchTag)
        else
            valorFlag(
                decoResult[Menudecoracao].value1,
                decoResult[Menudecoracao].value2,
                decoResult[Menudecoracao].value3,
                decoResult[Menudecoracao].value4,
                decoResult[Menudecoracao].value5,
                decoResult[Menudecoracao].value6,
                decoResult[Menudecoracao].value7,
                decoResult[Menudecoracao].flagsOn
            )
            reflashFlags(SearchTag)
        end
    end
...
function itensMenu()
    local MenuItens = gg.choice(itensChoice, nil, "ϟSelecione um item!ϟ")
    if MenuItens == nil then
    else
        if MenuItens == tablelength(itensChoice) then
            reflashFlags(SearchTag)
        else
            valorFlag(
                itensResult[MenuItens].value1,
                itensResult[MenuItens].value2,
                itensResult[MenuItens].value3,
                itensResult[MenuItens].value4,
                itensResult[MenuItens].value5,
                itensResult[MenuItens].value6,
                itensResult[MenuItens].value7,
                itensResult[MenuItens].flagsOn
            )
            reflashFlags(SearchTag)
        end
    end
...
function fotoMenu()
    local menuFoto = gg.choice(fotoChoice, nil, "ϟSelecione uma Foto!ϟ")
    if menuFoto == nil then
    else
        if menuFoto == tablelength(fotoChoice) then
            reflashFlags(SearchTag)
        else
            valorFlag(
                fotoResult[menuFoto].value1,
                fotoResult[menuFoto].value2,
                fotoResult[menuFoto].value3,
                fotoResult[menuFoto].value4,
                fotoResult[menuFoto].value5,
